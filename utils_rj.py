...
def save_model(model, total, args):
    ...
    """
    pth_name = args.arch + "_epochs_" + str(args.num_epochs) + "_dataset_" + str(
        total) + "_weights_" + datetime.datetime.now().strftime("%Y-%m-%d_%H:%M") + ".pth"
    if (args.folder == ""):
...
def save_epochs_results(epochs_details_list, total, n, args):
    ...
    """
    csv_name = args.arch + "_epochs_" + str(args.num_epochs) + "_dataset_" + str(
        total) +"ss_"+str(n)+ "_details_" + datetime.datetime.now().strftime("%Y-%m-%d %H:%M") + ".csv"
...
def save_best_epoch_results(results_detailed_list, total, args):
    ...
    csv_name = args.arch + "_epochs_" + str(args.num_epochs) + "_dataset_" + str(
        total) + "_details_best_epoch_" + datetime.datetime.now().strftime("%Y-%m-%d %H:%M") + ".csv"
    df.to_csv(csv_name)
